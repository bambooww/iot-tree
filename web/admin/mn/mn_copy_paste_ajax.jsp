...
%><%
if(!Convert.checkReqEmpty(request, out, "op","container_id","netid"))
	return ;
String cur_cont_id = request.getParameter("container_id") ;
String cur_netid = request.getParameter("netid") ;
...
String t = request.getParameter("t") ;
	...
	case "copy":
		...
		if(itemjson!=null)
		{
			JSONArray jarr = new JSONArray(itemjson) ;
			if(jarr.length()!=1)
				return ;
			JSONObject tmpjo = jarr.getJSONObject(0) ;
			tmpjo.put("_cp_container_id",cur_cont_id) ;
			tmpjo.put("_cp_netid",cur_netid) ;
			session.setAttribute("items_json", jarr) ;
		}
		break;
	case "paste":
		JSONArray tmps = (JSONArray)session.getAttribute("items_json");
		if(tmps!=null)
			out.print(tmps) ;
		break;
	case "common_copy":
