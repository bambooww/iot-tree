...
<div class="toolbar" style="width:100%;height:30px;line-height:30px;border:1px solid #ccc;text-align: center;">
	<wbt:g>node</wbt:g>:[<%=title %>] <wbt:g>runblk,list</wbt:g>
...
<table id="runblk_ins_list"  lay-filter="runblk_ins_list"  lay-size="sm" lay-even="true" style="top:1px;width:99%;">

</table>
<fieldset class="layui-elem-field">
  <legend><wbt:g>runblk,detail,setup</wbt:g>-<span id="cur_blk"></span></legend>
  <div class="layui-field-box" id="runblk_detail" style="overflow: hidden;">
  <iframe id="if_blk_detail" style="width:100%;height:300px;border:0px;overflow: hidden;"></iframe>
	</div>
 </fieldset>
</body>
...
function on_sel_blk(blkins)
{
	//console.log(blkins);
	$("#if_blk_detail").attr("src",`../util/runblk_ins_detail_set.jsp?treeid=\${treeid}&tree_nid=\${tree_nid}&ins_name=\${blkins.ins_name}`) ;
}
...
function updown_runblk(blk,b_up)
{
	send_ajax("tn_runblk_ajax.jsp",{op:"updown_runblk",cuid:cuid,tree_nid:tree_nid,runblk_id:blk.id,updown:b_up},function(bsucc,ret){
		if(!bsucc || ret!='succ')
		{
			dlg.msg(ret) ;
			return ;
...
function on_single_node_seled(node)
{
	if(!node)
	{
		location.href="dt_tree_tn_runblks.jsp?treeid="+treeid;
		return;
	}
	location.href="dt_tree_tn_runblks.jsp?treeid="+treeid+"&tree_nid="+node.id;
}
