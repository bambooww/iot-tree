...
<%@ page import="java.util.*,
			...
			{
					File f = new File(Config.getDataDirBase()+"/iott/unit/u_"+id+".txt") ;
					if(!f.exists())
					{
						return null;
					}
					String txt= Convert.readFileTxt(f, "UTF-8") ;
					f = new File(Config.getDataDirBase()+"/iott/unit/u_"+id+".ext") ;
					if(f.exists())
					{
	...
	File dirf = new File(Config.getDataDirBase()+"/iott/unit/") ;
	if(!dirf.exists())
		dirf.mkdirs();
