...
## JSON To Table（JSON对象写入到数据库表）

此节点内部通过定义数据库表结构——每个列定义及对应JSON属性映射关系。

当有json对象输入时，节点自动根据配置对json对象提取相关属性，形成一条数据库记录，并插入到数据库中。

此节点必须把“Relational DB Table”端子关联到一个DB Table节点，并且支持自动建立数据库表。
