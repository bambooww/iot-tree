<%@ page contentType="text/html;charset=UTF-8" isELIgnored="false"%><%@ page
	import="java.util.*,
				java.io.*,
				java.net.*,
				org.iottree.core.*,
				org.iottree.core.util.*
				"%><%
	String ver = Config.getVersion() ;
	boolean bsimple = "true".equals(request.getParameter("simple"));
	boolean oc = "true".equals(request.getParameter("oc"));
	boolean oc_min= "true".equals(request.getParameter("oc_min"));
	boolean tree  ="true".equals(request.getParameter("tree"));
	
	String path_head = request.getParameter("path_head") ;// for nginx proxy_pass supporting
	if(Convert.isNullOrEmpty(path_head))
	{
		path_head = "" ;
	}
	else
	{
		if(!path_head.startsWith("/"))
			path_head = "/"+path_head;
	}
				
	if(bsimple)
	{
%>
<script src="<%=path_head %>/_js/jquery-1.12.0.min.js"></script>
<link rel="stylesheet" type="text/css" href="<%=path_head %>/_js/layui/css/layui.css" />
<script type="text/javascript" src="<%=path_head %>/_js/ajax.js"></script>
<script src="<%=path_head %>/_js/layui/layui.all.js"></script>
<script src="<%=path_head %>/_js/dlg_layer.js"></script>
<script defer src="<%=path_head %>/_js/font6/js/all.js"></script>
<link href="<%=path_head %>/_js/font6/css/all.css" rel="stylesheet">
<%
	}
	else
	{
%><script src="<%=path_head %>/_js/jquery-1.12.0.min.js"></script>
<script src="<%=path_head %>/_js/bootstrap/js/bootstrap.min.js"></script>
<script type="text/javascript" src="<%=path_head %>/_js/ajax.js"></script>
<link rel="stylesheet" type="text/css" href="<%=path_head %>/_js/layui/css/layui.css" />
<script src="<%=path_head %>/_js/layui/layui.all.js"></script>
<script src="<%=path_head %>/_js/dlg_layer.js?v=<%=ver%>"></script>
<link  href="<%=path_head %>/_js/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" >
<script defer src="<%=path_head %>/_js/font6/js/all.js"></script>
<link href="<%=path_head %>/_js/font6/css/all.css" rel="stylesheet">
<script src="<%=path_head %>/admin/js/util.js"></script>
<%--
<link  href="/_js/font4.7.0/css/font-awesome.css"  rel="stylesheet" type="text/css" >
 --%>
<%
	}
if(oc)
{
	File f = new File(Config.getWebappBase()+"/_js/oc/oc.js") ;
	long dt = 0 ;
	if(f.exists())
		dt = f.lastModified() ;
	//System.out.println("dt="+dt) ;
%>
<script src="<%=path_head %>/_js/oc/hmi_util.js?v=<%=ver%>"></script>
<script src="<%=path_head %>/_js/oc/oc.js?v=<%=dt%>"></script>
<script src="<%=path_head %>/_js/oc/oc_util.js?v=<%=ver%>"></script>
<link type="text/css" href="<%=path_head %>/_js/oc/oc.css?v=<%=ver%>" rel="stylesheet" />

<%
}
if(oc_min)
{
	File f = new File(Config.getWebappBase()+"/_js/oc/oc.min.js") ;
	long dt = 0 ;
	if(f.exists())
		dt = f.lastModified() ;
	//System.out.println("dt="+dt) ;
%>
<script src="<%=path_head %>/_js/oc/oc.min.js?v=<%=dt%>"></script>
<script src="<%=path_head %>/_js/oc/oc_util.js?v=<%=ver%>"></script>
<link type="text/css" href="<%=path_head %>/_js/oc/oc.css?v=<%=ver%>" rel="stylesheet" />
<%
}
if(tree)
{
%>
<script src="<%=path_head %>/_js/jstree/jstree.min.js"></script>
<link rel="stylesheet" href="<%=path_head %>/_js/jstree/themes/default/style.min.css" />
<link type="text/css" href="<%=path_head %>/admin/inc/tree.css" rel="stylesheet" />
<%
}
%>