...
public class FinsAddr extends DevAddr implements Comparable<FinsAddr>
{
	...
	transient FinsAddrDef addrDef = null ;
	
	transient FinsAddrSeg addrSeg = null ;
	
	public FinsAddr()
	{}
	
	protected FinsAddr(ValTP vtp,String prefix,int addr_num,int bit_num,int digit_num)
	{
	...
	FinsAddr asDef(FinsAddrDef addr_def,FinsAddrSeg seg)
	{
	...
	public FinsAddrDef getAddrDef()
	{
		return this.addrDef ;
	}
	
	public FinsAddrSeg getAddrSeg()
	{
		return this.addrSeg ;
	}
...
//	
	...
	public DevAddr parseAddr(UADev dev,String str, ValTP vtp, StringBuilder failedr)
	{
		...
		FinsModel fx_m = (FinsModel)dev.getDrvDevModel() ;
		if(fx_m==null)
			throw new IllegalArgumentException("no FinsModel") ;
	...
	public ChkRes checkAddr(UADev dev,String addr,ValTP vtp)
	{
		...
		if(ss==null)
			return new ChkRes(-1,addr,vtp,"Invalid FinsAddr="+addr);
		FinsModel fxm = (FinsModel)dev.getDrvDevModel() ;
		
		FinsAddrDef addrdef = fxm.getAddrDef(ss.get(0)) ;
		if(addrdef==null)
		{
			return new ChkRes(-1,addr,vtp,"Invalid FinsAddr no address def found");
		}
		if(vtp==null)
		{
			return new ChkRes(-1,addr,vtp,"Invalid FinsAddr no ValTP found");
		}
		FinsAddrSeg seg = addrdef.findSeg(vtp,ss.get(1)) ;
		if(seg==null)
		{
			return new ChkRes(-1,addr,vtp,"FinsAddr ["+ss.get(0)+ss.get(1)+"] is invalid or not match "+vtp.getStr());
		}
	...
	@Override
	public int compareTo(FinsAddr o)
	{
